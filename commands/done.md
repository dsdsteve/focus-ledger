...
Steps:
1. If `$ARGUMENTS` is empty, ask which thread to mark done and stop.
2. Pass `$ARGUMENTS` as one argv value using safe shell quoting and run `${CLAUDE_PLUGIN_ROOT}/scripts/focus-done.sh` with Bash. Never interpolate the query into executable shell syntax. Retain the exit code and stdout.
