...
Duration requested: the duration the user gave, such as `4h` or `2d`. If they gave none, pass no argument and the script uses `1d`.

Pass the duration as one argv value using safe shell quoting and run `${CLAUDE_PLUGIN_ROOT}/scripts/focus-snooze.sh` with Bash. Never interpolate the duration into executable shell syntax. Retain the exit code and stdout.
